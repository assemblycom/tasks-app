...
export class CopilotAPI {
  ...
  async _getTokenPayload(): Promise<Token | null> {
    ...
    if (!getTokenPayload) {
      console.warn('CopilotAPI#getTokenPayload | Could not parse token payload')
      return null
    }
  ...
  async _getClientNotifications(
    ...
    if (!APP_ID) {
      console.warn('CopilotAPI#getClientNotifications | Skipping notification lookup because app id is not configured')
      return []
    }
