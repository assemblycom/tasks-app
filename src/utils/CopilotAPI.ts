...
export class CopilotAPI {
  ...
  private async _getBaseUrl(): Promise<string> {
    ...
    const tokenPayload = assembly.getTokenPayload ? await assembly.getTokenPayload() : null
    const baseUrl = tokenPayload?.baseUrl || assemblyApiDomain
    if (!baseUrl) {
      throw new Error('No API base URL available: set NEXT_PUBLIC_ASSEMBLY_API_DOMAIN or provide a token with baseUrl')
    }
    return baseUrl
  }
  ...
  private async _manualFetch(route: string, query?: Record<string, string>, workspaceId?: string) {
    ...
    const url = new URL(`${baseUrl}/v1/${route}`)
    if (query) {
  ...
  async dispatchWebhook(eventName: DISPATCHABLE_EVENT, { workspaceId, payload }: { workspaceId: string; payload?: object }) {
    ...
    console.info('CopilotAPI#dispatchWebhook | Dispatching webhook', eventName, 'with payload', payload ?? null)

    try {
      const assembly = await this.assemblyPromise
      await assembly.sendWebhook(eventName, payload)
    } catch (e) {
